...
from gnns.NodeGCN_classify import NodeGCN as NodeGCN_cls
...
class ModelSelector:
    ...
    def select_gnn_model(self):
        if self.model_name == 'GraphGCN':
            ...
            elif self.dataset_name == 'bareg3':
                return RegGraphGCN('train', 10, 20)
            ...
            elif self.dataset_name == 'benzene':
                return GraphGCN_cls('train', 14, 2)
            elif self.dataset_name == 'flca':
                return GraphGCN_cls('train', 14, 2)
            elif self.dataset_name == 'alca':
                return GraphGCN_cls('train', 14, 2)
            elif self.dataset_name == 'mutag':
                return GraphGCN_cls('train', 14, 2)
            elif self.dataset_name == 'ba2motif':
                return GraphGCN_cls('train', 10, 2)
            elif self.dataset_name in ['syn1']:
                return GraphGCN_cls('train', 10, 4)
            elif self.dataset_name in ['syn2']:
                return GraphGCN_cls('train', 10, 8)
            elif self.dataset_name in ['syn3']:
                return GraphGCN_cls('train', 10, 2)
            elif self.dataset_name in ['syn4']:
                return GraphGCN_cls('train', 10, 2)
        elif self.model_name == 'NodeGCN':
            if self.dataset_name in ['syn1']:
                return NodeGCN_cls('train', 10, 4)
            elif self.dataset_name in ['syn2']:
                return NodeGCN_cls('train', 10, 8)
            elif self.dataset_name in ['syn3']:
                return NodeGCN_cls('train', 10, 2)
            elif self.dataset_name in ['syn4']:
                return NodeGCN_cls('train', 10, 2)
    ...
    def load_best_model(self, best_epoch, eval_enabled):
        ...
        print(
            f"This model obtained: Train Acc: {checkpoint['train_acc']:.4f}, Val Acc: {checkpoint['val_acc']:.4f}.")
        if eval_enabled:
            self.model.eval()
