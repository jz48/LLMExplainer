...
class PGExplainer(BaseExplainer):
    ...
    def train(self, indices=None):
        ...
        for e in tqdm(range(0, self.epochs)):
            ...
            for n in indices:
                ...
                self.train_res_dict[e]['explanations'].append([n, graph.detach().numpy().tolist(), expl_graph_weights.detach().numpy().tolist()])
                '''
    ...
    def train_1_epoch(self, e, indices=None):
        ...
        # print(e, loss)
        loss = loss.to(torch.float32)
        train_loss = loss.detach().numpy().tolist()
        loss.backward()
