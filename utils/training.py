...
class GNNTrainer:
    def __init__(self, dataset_name, model_name, lr=0.001, epochs=3000, clip_max=2.0, batch_size=64, early_stopping=500,
        ...
        if self.dataset_name.startswith('syn'):
            self.batch_size = 64
        ...
        if self.dataset_name == 'mutag':
            # self.model_manager.model.learning_rate = 0.001
            pass
        pass

    def _train_graph(self, _dataset, _paper, args):
        """
    ...
    def evaluate_rmse(self, out, labels):
        """
    ...
    def evaluate_acc(self, out, labels):
        """
            Calculates the accuracy between the prediction and the ground truth.
            :param out: predicted outputs of the explainer
            :param labels: ground truth of the data
    ...
    def train_node(self):
        ...
        for epoch in range(0, self.epochs):
            model.train()
            optimizer.zero_grad()
            out = model(self.dataset_loader.train_data_list.x, self.dataset_loader.train_data_list.edge_index)
            loss = criterion(out[self.dataset_loader.train_mask], self.dataset_loader.train_data_list.y[self.dataset_loader.train_mask])
            loss.backward()
            ...
            # Evaluate train
            train_acc = self.evaluate_acc(out[self.dataset_loader.train_mask], self.dataset_loader.train_data_list.y[self.dataset_loader.train_mask])
            test_acc = self.evaluate_acc(out[self.dataset_loader.test_mask], self.dataset_loader.train_data_list.y[self.dataset_loader.test_mask])
            val_acc = self.evaluate_acc(out[self.dataset_loader.val_mask], self.dataset_loader.train_data_list.y[self.dataset_loader.val_mask])
            ...
            if val_acc > best_val_acc:  # New best results
                ...
                self.model_manager.store_checkpoint(self.model_name, self.dataset_name, model, train_acc, val_acc,
                                                best_epoch)

            if epoch - best_epoch > self.early_stopping and best_val_acc > 0.99:
                break

        self.model_manager.load_best_model(best_epoch, self.eval_enabled)
        model = self.model_manager.model

        out = model(self.dataset_loader.train_data_list.x, self.dataset_loader.train_data_list.edge_index)

        # Train eval
        train_acc = self.evaluate_acc(out[self.dataset_loader.train_mask],
                                      self.dataset_loader.train_data_list.y[self.dataset_loader.train_mask])
        test_acc = self.evaluate_acc(out[self.dataset_loader.test_mask],
                                     self.dataset_loader.train_data_list.y[self.dataset_loader.test_mask])
        val_acc = self.evaluate_acc(out[self.dataset_loader.val_mask],
                                    self.dataset_loader.train_data_list.y[self.dataset_loader.val_mask])
        ...
        if model.type == 'reg':
            ...
            self.model_manager.store_best_checkpoint(self.model_name, self.dataset_name, model, train_acc, val_acc,
                                                     best_epoch)
        ...
        for epoch in range(0, self.epochs):
            ...
            if self.type == 'reg':
                ...
                with torch.no_grad():
                    ...
                    train_loss = float(train_loss) / int(train_len)
                    ...
                    def do_val_alt(the_loader, the_len):
                        ...
                        for data in the_loader:
                            ...
                            out = torch.concat((out, 1 - out), dim=1)
                            y_pred = torch.argmax(out, dim=1)
                            acc += (y_pred == torch.tensor(data.y, dtype=torch.long)).sum().item()
                        # print('val mape: ', val_mape, len(val_set_alt))
                        the_loss = float(the_loss) / the_len
                        acc = float(acc) / the_len
                        return the_loss, acc

                    # val_loss, acc = do_val_alt(train_loader, int(len(train_loader)))
                    # print('train loss and acc: ', val_loss, acc)

                    val_loss, acc = do_val_alt(val_loader, int(len(val_loader)))
                    # print('val loss and acc', val_loss, acc)

                if acc > best_acc:  # New best results  # val_loss < best_val_loss and
                    if self.logging:
                        print("Val improved")
                        print(f"Epoch: {epoch}, train_loss: {train_loss:.6f}, val_loss: {val_loss:.6f}, acc: {acc:.6f}")
                ...
                # Early stopping
                if epoch - best_epoch > self.early_stopping:  # or val_loss < 0.000001:
                    break
                pass
            else:
                assert 0
