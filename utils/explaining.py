...
class MainExplainer:
    ...
    def explain(self):
        ...
        get_distribution_repair_degree = False
        if get_distribution_repair_degree:
            self.seeds = [0]
        ...
        if get_distribution_repair_degree:
            data = self.explainer_manager.explainer.distrition_repair_degree
            # for i in range(len(self.dataset_loader.edge_ground_truth)):
            #     expl = self.dataset_loader.edge_ground_truth[i]
            #     expl = torch.tensor(expl)
            #     feature = self.dataset_loader.features[i]
            #     feature = torch.tensor(feature)
            #     # edge_weights = torch.tensor()
            #     emb = self.explainer_manager.explainer.model_to_explain.embedding(expl, feature)
            #     pred = self.explainer_manager.explainer.model_to_explain(expl, feature)
            #     data[1].append([emb, pred])

            with open(os.path.join('./data/', self.dataset_name + '_distribution_repair_degree.json'), 'w') as f:
